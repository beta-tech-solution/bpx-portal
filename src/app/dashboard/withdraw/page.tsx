...
export default function WithdrawPage() {
  ...
  useEffect(() => {
    ...
    // One-time check for pending withdrawals
    const checkPendingStatus = async () => {
      ...
      try {
        ...
        toast({
          title: "Error",
          description: "Could not check withdrawal status.",
          variant: "destructive",
        });
      } finally {
        setCheckingStatus(false);
    ...
    // Listener for user data (to check status and BPExch credentials)
    ...
    // Listener for approved withdrawals total
  ...
  const handleWithdraw = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (isNaN(amount) || amount < 100) {
      toast({
        ...
        description: "Please enter a valid withdrawal amount of at least PKR 100.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      toast({
        ...
        description: `Your request to withdraw PKR ${amount.toFixed(
          2
        )} has been received.`,
      });
  ...
  if (hasPendingWithdrawal) {
    return (
      <Alert variant="destructive" className="animate-fade-in">
        ...
        <AlertDescription>
          You already have a withdrawal request pending approval. Please wait
          for the admin to process it before submitting a new one.
        </AlertDescription>
      </Alert>
    );
  }
      ...
      {userData &&
        ...
        !hasPendingWithdrawal && (
          <Alert variant="destructive">
            ...
            <AlertDescription>
              Your account must be fully activated by an admin (with BPExch
              credentials assigned) before you can make a withdrawal request.
            </AlertDescription>
          </Alert>
        )}
      <Card className="w-full transition-shadow hover:shadow-lg">
        <form onSubmit={handleWithdraw}>
            ...
            <div className="space-y-6">
              <div className="p-4 rounded-lg border bg-muted/50">
                ...
                <p className="text-3xl font-bold text-primary">
                  PKR {totalWithdrawals.toFixed(2)}
                </p>
              </div>
              <div>
                <Label
                ...
                <Input
                  ...
                  min="100"
                  step="0.01"
                  className="mt-2 text-lg font-bold h-12 p-3"
                />
              </div>
              <div>
            ...
            <div className="p-4 border rounded-md space-y-6">
              <div>
                ...
                <ul className="list-disc list-inside space-y-1 text-sm text-muted-foreground">
                  ...
                  <li>
                    Ensure all account details are correct to avoid delays.
                  </li>
                </ul>
              </div>
              <div className="text-right">
                ...
                <ul className="list-disc list-inside space-y-1 rtl font-code">
                  ...
                  <li>
                    تاخیر سے بچنے کے لیے یقینی بنائیں کہ اکاؤنٹ کی تمام تفصیلات
                    درست ہیں۔
                  </li>
                </ul>
              </div>
          ...
          <CardFooter className="p-6 pt-0">
            <Button
              type="submit"
              className="w-full h-12 text-base font-bold bg-slate-800 hover:bg-slate-700 text-white"
              disabled={isLoading || isWithdrawalDisabled}
            >
              {isLoading ? (
